...
import {initialQuery,compile,allColumns,translateQuery,inspectTables} from './query.js';
...
function drawNodes(){
  ...
  $('nodes').innerHTML=tables.map(t=>{
    ...
    return `<article class="node ${(mode==='practice'?compiled?.connected.includes(t.name):productionTables.has(t.name))?'in-query':''}" id="node-${t.name}" style="left:${pos.x}px;top:${pos.y}px"><div class="node-head" data-drag="${t.name}"><strong>${t.name}</strong><small>${t.label}</small><button data-view="${t.name}" aria-label="${t.name}を${views[t.name]?'列一覧':'表'}に切り替え">${views[t.name]?'列一覧':'表を見る'}</button></div>${rows}<div class="node-foot">${views[t.name]?`先頭3行 / 全${t.rows.length}行`:'列名を選択してSELECTへ追加'}${t.name===q.from?' · 開始テーブル':''}</div></article>`;
  }).join('');
  drawLines();
...
function drawLines(){
 $('connections').innerHTML=relations.map(r=>{const a=positions[r.from],b=positions[r.to];const x=a.x+300,y=a.y+65,xx=b.x,yy=b.y+65;const active=mode==='practice'?compiled?.joins.some(e=>e.r===r):productionTables.has(r.from)&&productionTables.has(r.to);return `<path class="${active?'active':''}" d="M${x} ${y} C${x+35} ${y},${xx-35} ${yy},${xx} ${yy}"/><text x="${x+8}" y="${Math.min(y,yy)-12}">${r.key} → ${r.foreign}</text>`;}).join('');
}
...
function renderTranslation(){if(mode==='production'){$('translation').innerHTML='<strong>SQL → 図</strong>　「図へ反映」で、FROMとJOINに登場する既知テーブルのつながりを強調します。';return;}if(!compiled){$('translation').textContent='列を選ぶと、SQLの意味をここに日本語で表示します。';return;}$('translation').innerHTML=`<strong>このSQLの意味</strong>　${escape(translateQuery(q,compiled))}`;}
const currentSql=()=>mode==='production'?$('sql-editor').value.trim():compiled?.sql;
function startWorker(){worker=new Worker(new URL('./worker.js',import.meta.url));worker.onmessage=({data})=>{if(data.id!==sequence)return;clearTimeout(timer);$('run').disabled=mode==='practice'?!compiled:false;if(data.error){$('result-status').textContent=data.error;$('result').replaceChildren();$('count').textContent='エラー';$('result-count').textContent='!';return;} const result=data.result[0];const rows=result?.values??[];$('count').textContent=`${rows.length}行`;$('result-count').textContent=rows.length;$('result-status').textContent=lastExecuted===currentSql()?'実行済み · 教材DBの結果':'SQLが変わりました。再実行してください。';$('result').innerHTML=result?`<table><thead><tr>${result.columns.map(c=>`<th>${escape(c)}</th>`).join('')}</tr></thead><tbody>${rows.map(row=>`<tr>${row.map(v=>`<td>${escape(v)}</td>`).join('')}</tr>`).join('')}</tbody></table>${!rows.length?'<p class="empty">条件に一致する行はありません。</p>':''}`:'<p class="empty">結果はありません。</p>';};worker.onerror=event=>{clearTimeout(timer);$('run').disabled=mode==='practice'?!compiled:false;$('result-status').textContent=`SQLエンジンの読み込みに失敗しました${event.message?`：${event.message}`:'。ページを再読み込みしてください。'}`;$('count').textContent='エラー';$('result-count').textContent='!';};}
...
function run(openResult=false){const sql=currentSql();if(!sql)return;if(mode==='production'&&!/^\s*(SELECT|WITH)\b/i.test(sql)){$('notice').textContent='本番モードの実行はSELECTまたはWITHから始まるSQLに限定しています。';return;}clearTimeout(timer);const id=++sequence;lastExecuted=sql;$('run').disabled=true;$('result-status').textContent='実行中…';worker.postMessage({id,sql,params:mode==='practice'?compiled.params:[],tables,relations});if(openResult)showPane('result');timer=setTimeout(()=>{worker.terminate();startWorker();$('run').disabled=mode==='practice'?!compiled:false;$('result-status').textContent='実行を中止しました（10秒）。もう一度実行できます。';},10000);}
$('nodes').addEventListener('click',e=>{const view=e.target.closest('[data-view]');if(view){views[view.dataset.view]=!views[view.dataset.view];drawNodes();return;}if(mode==='production')return;const column=e.target.closest('[data-column]');if(column){const id=column.dataset.column;q.columns=q.columns.includes(id)?q.columns.filter(c=>c!==id):[...q.columns,id];update();return;}const cell=e.target.closest('[data-cell]');if(cell){const [name,i,j]=cell.dataset.cell.split(':');const t=tables.find(t=>t.name===name);q.filters.push({column:`${name}.${t.columns[j][0]}`,operator:'=',value:String(t.rows[i][j])});drawFilters();update();}});
...
function setNavState(next){const expanded=next==='expanded';bottomNav.dataset.navState=next;navCollapseToggle.setAttribute('aria-expanded',String(expanded));navCollapseToggle.setAttribute('aria-label',expanded?'ナビゲーションをしまう':'ナビゲーションを表示');}
navCollapseToggle.onclick=()=>setNavState(bottomNav.dataset.navState==='collapsed'?'expanded':'collapsed');
function setMode(next){mode=next;document.body.classList.toggle('mode-practice',mode==='practice');document.body.classList.toggle('mode-production',mode==='production');$('practice-mode').setAttribute('aria-pressed',String(mode==='practice'));$('production-mode').setAttribute('aria-pressed',String(mode==='production'));$('sql').hidden=mode==='production';$('sql-editor').hidden=mode==='practice';$('inspect-sql').hidden=mode==='practice';$('selection-count').hidden=mode==='production';$('run').disabled=mode==='practice'?!compiled:false;$('sql-kicker').textContent=mode==='practice'?'GENERATED QUERY':'SQL WORKBENCH';$('sql-title').textContent=mode==='practice'?'触った内容がSQLになる':'SQLからデータのつながりを読む';$('canvas-title').textContent=mode==='practice'?'欲しいデータを選ぶ':'SQLに登場するデータを見る';$('canvas-guide').textContent=mode==='practice'?'列名で表示項目、セルの値で絞り込みを選択':'FROMとJOINに現れるテーブルをキャンバスで強調';if(mode==='production'&&!$('sql-editor').value)$('sql-editor').value=compiled?.display??'';if(mode==='practice')productionTables.clear();renderTranslation();drawNodes();showPane('sql');}
$('practice-mode').onclick=()=>setMode('practice');$('production-mode').onclick=()=>setMode('production');
$('inspect-sql').onclick=()=>{const sql=$('sql-editor').value;productionTables=new Set(inspectTables(sql,tables));drawNodes();if(!productionTables.size){$('notice').textContent='既知のテーブルを見つけられませんでした。';return;}$('notice').textContent=`${[...productionTables].join(' → ')} のつながりを図へ反映しました。${/\(\s*SELECT\b/i.test(sql)?' サブクエリ内部の階層表示は次段階です。':''}`;};
$('sql-editor').addEventListener('input',()=>{$('notice').textContent='';$('result-status').textContent='SQLが変わりました。「SQLを実行」で結果を更新します。';});
